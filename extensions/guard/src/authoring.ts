...
export async function resolveGuardToolSignatures(api: OpenClawPluginApi): Promise<string> {
  try {
    const { resolvePluginTools } = (await import("../../../src/plugins/tools.js")) as {
      resolvePluginTools: (params: Record<string, unknown>) => PluginToolDescriptor[];
    };
    const pluginTools = resolvePluginTools({
      context: {
        config: api.config,
    ...
    const guardTools = pluginTools
      .filter((tool) => tool.name.startsWith("guard_"))
      .map((tool) => ({
        name: tool.name,
    ...
    return formatGuardToolSignatures(guardTools);
  } catch {
    return [
      "## Guard Tool Signatures",
      "",
      "Unable to resolve guard_* parameter schemas at runtime. If a tool call fails with parameter shape errors, inspect the tool schema in the UI and retry with the exact required fields.",
      "",
    ].join("\n");
  }
}
...
function formatGuardToolSignatures(
  entries: Array<{ name: string; description: string; parameters?: unknown }>,
): string {
  const lines: string[] = [
    "## Guard Tool Signatures",
    "",
    "These are the exact parameter schemas for the guard_* tools available to you. Use these field names and required properties exactly when calling tools.",
    "",
  ];

  if (entries.length === 0) {
    lines.push("No guard_* tool signatures were discovered.");
    lines.push("");
    return lines.join("\n");
  }
...
type AuthoringPromptParams = {
  mode: "create" | "edit";
  scheme?: unknown;
  introspectionSpec: unknown;
  toolCatalog: string;
  guardToolSignatures: string;
};
...
- Ask the user before compiling — they should confirm the scheme looks correct.
...
`);
  ...
  if (params.mode === "edit" && params.scheme) {
    parts.push(`## Current Scheme (Edit Mode)
...
You are creating a new scheme from scratch. Follow the full workflow: understand intent → inventory tools → build knowledge graph → draft rules → validate → simulate → compile.
`);
  }

  parts.push(params.toolCatalog);
  parts.push("");
  parts.push(params.guardToolSignatures);
  parts.push("");

  // Include the introspection spec as reference (it's large but critical)
  parts.push(`## Guard Rule Type Specification

The following is the complete Guard introspection spec. Refer to it for rule type details, required fields, composition patterns, and anti-patterns.

\`\`\`json
${JSON.stringify(params.introspectionSpec, null, 2)}
\`\`\`
`);
...
export class AuthoringSessionManager {
  ...
  async startSession(params: {
    ...
    const guardToolSignatures = await resolveGuardToolSignatures(this.api);

    // Fetch introspection spec
    const introspectionSpec = (await this.guardFetch("/v1/rules/spec", "GET")) ?? {};

    // Fetch current scheme if editing
  ...
  async sendMessage(
    ...
    const systemPrompt = buildAuthoringSystemPrompt({
      ...
      introspectionSpec: session.introspectionSpec,
      toolCatalog: session.toolCatalog,
      guardToolSignatures: session.guardToolSignatures,
    });
